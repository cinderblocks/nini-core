...
namespace Nini.Ini
{
	...
	public class IniReader : IDisposable
	{
		...
		bool disposed = false;
        char[] commentDelimiters = new[] { ';' };
		char[] assignDelimiters = new[] { '=' };
		#endregion
		...
		public string Comment => (hasComment) ? this.comment.ToString () : null;
		...
		public bool MoveToNextSection ()
		{
			...
			while (true)
			{
				...
				if (Type == IniType.Section || !result) {
					break;
				}
			}
			...
			while (true)
			{
				...
				if (EndOfLine (ch)) {
					...
					throw new IniException (this,
                        $"Expected assignment operator ({assignDelimiters[0]})");
				}
